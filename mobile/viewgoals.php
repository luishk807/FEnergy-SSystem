	  ...
	  if($result = mysql_query($query))
	  {
		  if(($num_rows=mysql_num_rows($result))>0)
		  {
			  while($row = mysql_fetch_array($result))
			  {
				...
				echo "<tr><td colspan='3' align='center' valign='middle'><hr/></td></tr><tr class='morow'><td align='center' valign='middle'><a href='accountgoals_set.php?id=".base64_encode($row["id"])."' class='contlinkb'>".getName($row["userid"])."</a></td><td align='center' valign='middle'>".fixdate_comps('invdate',$row["date"])."</td><td  align='center' valign='middle'>".$row["goals"]."</td></tr>";
			  }
		  }
		  else
		  	echo "<tr class='nfound_m'><td colspan='3' class='nfound_m'>No Users Found</td></tr>";
	  }
	  else
	  	echo "<tr class='nfound_m'><td colspan='3' class='nfound_m'>No Users Found</td></tr>";
	  ?>
