         ...
         if(strlen($queryString) >0)
		 {
            ...
            if($result = mysql_query($query))
			{
               while ($rows = mysql_fetch_array($result)) 
			   {
				   ...
				   {
				   ?>
				   <a href='javascript:addthis("<?php echo stripslashes($rows["name"]); ?>","<?Php echo stripslashes($rows["acode"]); ?>")'><?php echo stripslashes($rows["name"]); ?> &nbsp;[<span style="font-size:9pt;color:#FFF;font-family:'rockw'">Agent Code:&nbsp;<?php echo stripslashes($rows["acode"]); ?></span>] </a>
                   <?php
				   }
			   }
            } else {
