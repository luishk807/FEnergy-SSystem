...
if(sizeof($users)>0)
{
	for($i=0;$i <sizeof($users);$i++)
	{
		...
		$messageh .="<div style='font-size:18pt; text-align:center;'>From Date:<br/><u><b>".$todaypf." to ".$todaypt."</b></u><br/><span style='text-decoration:underline;'>Grand Total For<br/>".$oname.": <b>".$grandtotalo." </b></span></div>";
		$messageh .="<br/><br/><hr/>";
		//show office
		if(!empty($oname)) //if there is office name
		{
			...
			$message ="<table width='100%' border='0' cellspacing='0' cellpadding='0'><tr style='font-size:19pt; font-style:italic; font-weight:bold;'><td height='43' colspan='3' align='center' valign='middle'>".stripslashes($users[$i]["name"])."'s Team:<br/><b>$grandtotalm</b><br/>";
		...
		$fileName = "tmp/imagefile_xteam".$users[$i]["id"].".png";
		$graph->img->Stream($fileName);
		//add image to the php mailer
		$mail2->AddEmbeddedImage("tmp/imagefile_xteam".$users[$i]["id"].".png",md5($users[$i]["id"]).'_xteam-image','imagefile_xteam'.$users[$i]["id"].'.png');
		$message .="<img src='cid:".md5($users[$i]["id"])."_xteam-image' border='0' alt='graph for ".$users[$i]["name"]."_team'/>";*/
		...
		$massing="";
			...
			$message .="<tr><td height='34' colspan='3' align='center' valign='middle' style='font-size:14pt; font-family:Tahoma;'>[<span style='font-size:12pt; font-style:italic'>Mngr</span>] <b>".$users[$i]["name"]."</b> <br/>Total: ".$grandtotal."</td></tr>";
			...
			if(sizeof($weekamt)>0)
			{
				...
				if($checktotal>0)
				{
					...
					//$graph->Stroke();
					$graph->Stroke(_IMG_HANDLER);
					$fileName = "tmp/imagefile_xman".$users[$i]["id"].".png";
					$graph->img->Stream($fileName);
					//add image to the php mailer
					$mail2->AddEmbeddedImage("tmp/imagefile_xman".$users[$i]["id"].".png",md5($users[$i]["id"]).'xman-image','imagefile_xman'.$users[$i]["id"].'.png');
					$message .="<tr><td colspan='3' align='center' valign='middle'><img src='cid:".md5($users[$i]["id"])."xman-image' border='0' alt='graph for ".$users[$i]["name"]."'/></td></tr>";
				}
			}*/
		/***************END OF Script To Get Data For Graph***********************************************/
			...
			if($ry = mysql_query($qy))
			{
				if(($ny = mysql_num_rows($ry))>0)
				{
					...
					while($rowy = mysql_fetch_array($ry))
					{
						...
						$amstry="";*/
						$message .="<tr><td height='34' colspan='3' align='center' valign='middle' style='font-size:14pt; font-family:Tahoma;'><b>".$rowy["name"]."</b><br/>Total: ".$grandtotalx."</td></tr>";
				/***********Script To Get Data For Graph For Team Leader***************************************/
						...
						if($checktotal>0)
						{
							...
							//$graph->Stroke();
							$graph->Stroke(_IMG_HANDLER);
							$fileName = "tmp/imagefile_xteam".$rowy["id"].".png";
							$graph->img->Stream($fileName);
							//add image to the php mailer
							$mail2->AddEmbeddedImage("tmp/imagefile_xteam".$rowy["id"].".png",md5($rowy["id"]).'_xteam-image','imagefile'.$rowy["id"].'_xteam.png');
							$message .="<tr><td colspan='3' align='center' valign='middle'><img src='cid:".md5($rowy["id"])."_xteam-image' border='0' alt='graph for ".$rowy["name"]."'/></td></tr>";
						}*/
				/***********END OF Script To Get Data For Graph***********************************************/
						//get all agent sales from selected manager and office
		...
		if(!empty($users[$i]["email"]))
		{
			...
			//echo $grandmessage;
			$mail3=clone $mail2;
			$mail3->AddAddress($users[$i]["email"], $users[$i]["name"]);
			$mail3->Subject=$title;
			$mail3->Body=$grandmessage;
			$mail3->Send();
			sleep(5);
			if(sizeof($leadersinfo)>0)
			{
				for($x=0;$x<sizeof($leadersinfo);$x++)
				{
					...
					{
						$mail4=clone $mail2;
						$mail4->AddAddress($leadersinfo[$x]["email"], $users[$i]["name"]);
						$mail4->Subject=$title;
						$mail4->Body=$grandmessage;
						$mail4->Send();
						sleep(5);
					}
				}
