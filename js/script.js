...
function changeStatSort_real(value,qu,ascdesc)
{
	getHttpPost();
    xmlHttpReq.onreadystatechange = function()
	{
	...
	var url = "popupdategraph_r.php?task="+value+"&"+qu+"&ascdesc="+ascdesc;
	xmlHttpReq.open('POST',url, true);
	//xmlHttp.setRequestHeader("If-Modified-Since", "Sat, 1 Jan 2000 00:00:00 GMT");
    xmlHttpReq.send(null);
}
...
function showgraphinfopop_real(value)
{
	getHttpPost();
    xmlHttpReq.onreadystatechange = function() 
	{
	...
	var url = "showgraphresult_real.php?"+value;
	xmlHttpReq.open('POST',url, true);
	//xmlHttp.setRequestHeader("If-Modified-Since", "Sat, 1 Jan 2000 00:00:00 GMT");
    xmlHttpReq.send(null);
}
...
function checkField_excel()
{
	...
	{
		document.getElementById("importbtn").src="images/importbtn.jpg";
		return false;
	}
	else
		return true;
...
function checkFieldd_a()
{
	...
	if(!errorcheck("normal","aname","Please provide a valid agent name"))
		return false;
	//if(!errorcheck("normal","ucode","Please provide a valid agent code"))
	//	return false;
...
function checkFieldgraphdate_real()
{
	//form from graph view for date engine
	var xyear1=document.getElementById("xyear1").value;
	var xyear2=document.getElementById("xyear2").value;
	...
	else
	{
		...
		if(cho_monthx.length>0)
			imonthx="&cho_monthx="+cho_monthx;
		document.getElementById("message").innerHTML="";
		window.location.href="viewgraph_r.php?xyear1="+xyear1+"&xyear2="+xyear2+""+imonthx+""+ioffice;
	}
}
function checkField_ototal()
{
	if(!errorcheck("number","december_utotal","Please provide a valid number for December"))
		return false;
	if(!errorcheck("number","january_utotal","Please provide a valid number for January"))
		return false;
	if(!errorcheck("number","february_utotal","Please provide a valid number for February"))
		return false;
	if(!errorcheck("number","march_utotal","Please provide a valid number for March"))
		return false;
	if(!errorcheck("number","april_utotal","Please provide a valid number for April"))
		return false;
	if(!errorcheck("number","may_utotal","Please provide a valid number for May"))
		return false;
	if(!errorcheck("number","june_utotal","Please provide a valid number for June"))
		return false;
	if(!errorcheck("number","july_utotal","Please provide a valid number for July"))
		return false;
	if(!errorcheck("number","august_utotal","Please provide a valid number for August"))
		return false;
	if(!errorcheck("number","september_utotal","Please provide a valid number for September"))
		return false;
	if(!errorcheck("number","october_utotal","Please provide a valid number for October"))
		return false;
	if(!errorcheck("number","november_utotal","Please provide a valid number for November"))
		return false;
	return true;
}
...
function checkFieldagenti()
{
	//form from the agent edittion
	if(!errorcheck("normal","uname","Please provide a valid agent name"))
		return false;
	if(!errorcheck("normal","ucode","Please provide a valid agent code"))
		return false;
	var checkdup=document.getElementById("checkdup").value;
	if(checkdup=="yes")
	{
		var mdup=document.getElementById("mdup").checked;
		if(mdup==true)
		{
			if(!isChecked("agentform","mdup_in",true))
			{
				document.getElementById("message2").innerHTML="You need at least one agent to merge account";
				return false;
			}
			else
				document.getElementById("message2").innerHTML="";
...
function checkFieldv_ax_r()
{
	//form from the real report edit form browser
	if(!errorcheck("normal","aname","Please provide a valid agent name"))
		return false;
	if(!errorcheck("normal","ucode","Please provide a valid agent code"))
		return false;
	if(!errorcheck("selects","uoffice","Please select the office"))
		return false;
	if(!errorcheck("number","uelec","Please provide a valid total of electrical sales"))
		return false;
	if(!errorcheck("number","ugas","Please provide a valid total of gas sales"))
		return false;
	if(!errorcheck("normal","ddatex","Please provide a valid date of sales"))
		return false;
	return true;
}
...
function adddriver(value)
{
	...
	if(confirmx==true)
		window.location.href='http://www.familyenergymarketing.com/femcar/save.php?task=setcar&v='+value;	
}
function returnadd(value)
{
	...
	if(confirmx==true)
		window.location.href='http://www.familyenergymarketing.com/femcar/save.php?task=returncar&v='+value;	
}
...
{
	var host="http://www.familyenergymarketing.com/salesreport/";
	var check=false;
	var id=document.getElementById("id").value;
	var ogoalx=document.getElementById("ogoal").value;
...
function deletetask(tasks,value)
{
	...
	else if(tasks =="report_r")
	{
		var confirmx = window.confirm("WARNING!!: YOU ARE ABOUT TO DELETE THIS REPORT. ARE YOU SURE YOU WANT TO PROCEED?!\r\n\r\nClick Yes To Proceed Or Cancel To Cancel The Process.");
		if(confirmx==true)
		window.location.href=host+"save.php?task=deletereport_r&id="+value;	
	}
...
function isChecked(formname,checkfield,checkname)
{
	//check if the given checkbox is checked at least one
	var found=false;
	if(document.forms[formname])
	{
		var objCheckBoxes = document.forms[formname].elements[checkfield];
		if(objCheckBoxes)
		{
			...
			if(countCheckBoxes)
			{
				...
				{
					if(objCheckBoxes[i].checked == checkname)
					{
						found=true;
						break;
					}
...
function checkbox_marker(formname,checkfield,checkname)
{
	//mark or unmark a checkfield
	if(!document.forms[formname])
		return;
	var objCheckBoxes = document.forms[formname].elements[checkfield];
	if(!objCheckBoxes)
	...
	var countCheckBoxes = objCheckBoxes.length;
	if(!countCheckBoxes)
		objCheckBoxes.checked = checkname;
	else
		for(var i = 0; i < countCheckBoxes; i++)
			objCheckBoxes[i].checked = checkname;
}